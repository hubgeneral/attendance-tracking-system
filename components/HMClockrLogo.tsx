...
type HMClockrLogoProps = {
  size?: "small" | "medium" | "large";
  showText?: boolean;
};

export default function HMClockrLogo({ size = "medium", showText = true }: HMClockrLogoProps) {
  const getSizeConfig = (size: "small" | "medium" | "large") => {
    switch (size) {
      ...
      case "large":
        return { logoSize: 40, fontSize: 18 };
      default:
        return { logoSize: 32, fontSize: 16 };
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {showText && (
        <Text style={[styles.text, { fontSize: sizeConfig.fontSize }]}>HM Clockr</Text>
      )}
    </View>
  );
}
